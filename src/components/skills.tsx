      ...
      <div className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:gap-8">
        {SKILL_CATEGORIES.map((category) => (
          <article
            ...
            className="flex flex-col justify-between rounded-xl border border-border bg-surface p-6 shadow-2xs transition-colors hover:border-border/90 sm:p-7"
          >
            <div>
              {/* Category Header */}
            ...
            <div className="mt-6 border-t border-border/40 pt-4">
              ...
              >
                {category.skills.map((skill) => (
                  <li key={skill}>
                    <span className="inline-flex items-center rounded-md border border-border/60 bg-surface-elevated/70 px-3 py-1 font-mono text-xs font-medium text-text-primary">
                      {skill}
                    </span>
                  </li>
                ))}
